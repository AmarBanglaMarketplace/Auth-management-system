...
class ShopAdminAuthenticationController extends Controller
{
    /**
     * Register a new Shop Admin user.
     *
    ...
    public function register(Request $request)
    {
        ...
        $request->validate([
            'name'     => 'required|string|max:255',
            'email'    => 'required|email|unique:shop_admins,email',
            'password' => 'required|min:6|confirmed',
        ]);
        ...
        // Issue token
        $token = $user->createToken('shop_admins-token', ['*'])->plainTextToken;
    ...
    public function login(Request $request)
    {
        ...
        $token = $admin->createToken('shop_admins-token', ['*'])->plainTextToken;
     ...
     *
     * This endpoint deletes all active Sanctum tokens for the current Shop Admin,
     * effectively invalidating any existing sessions. A successful response
     * confirms that the Shop Admin has been logged out and must re‑authenticate
